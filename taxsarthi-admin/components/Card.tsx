...
const Card: React.FC<CardProps> = ({ icon, count, title, isActive }) => {
  ...
  return (
    <div className={`group relative border min-h-32 min-w-40 lg:min-w-60 shadow-sm bg-white flex flex-col md:flex-row justify-center gap-4 items-center rounded-lg p-4 md:p-6 transition-transform transform hover:scale-105 hover:shadow-lg cursor-pointer`}>
      <div className={`flex-shrink-0 text-5xl ${activeClass}`}>
        {icon}
      </div>
      <div className={`flex items-center justify-center flex-col ${activeClass}`}>
        <p className={`text-lg lg:text-xl font-medium ${activeClass}`}>{count}</p>
        <p className={`text-sm font-semibold lg:font-normal lg:text-lg ${activeClass}`}>{title}</p>
      </div>
